...
ImVec2 ImGui::ImageAndGetWindowPos(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return ImVec2(0,0);
...
bool ImGui::RangeSliderFloat(const char* label, float* v1, float* v2, float v_min, float v_max, const char* display_format, float power)
{
	...
	const bool hovered = IsItemHovered(0);
	if (hovered)
		SetHoveredID(id);
	...
	if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
		return InputScalarAsWidgetReplacement(frame_bb, id, label, ImGuiDataType_Float, v1/*, decimal_precision*/, display_format);

	ItemSize(total_bb, style.FramePadding.y);

	// Actual slider behavior + render grab
	bool value_changed = RangeSliderBehavior(frame_bb, id, v1, v2, v_min, v_max, power, decimal_precision);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        for (int i = 0; i < 6; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), 
				ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), hue_colors[i], hue_colors[i], hue_colors[i + 1], hue_colors[i + 1]);
...
//helper function
static void RenderArrowsForHorizontalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_h)
{
	ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + half_sz.y + 1), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Down, IM_COL32_BLACK);
	ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + half_sz.y), half_sz, ImGuiDir_Down, IM_COL32_WHITE);
	ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + bar_h - half_sz.y - 1), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Up, IM_COL32_BLACK);
	ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x , pos.y + bar_h - half_sz.y), half_sz, ImGuiDir_Up, IM_COL32_WHITE);
}
// Note: ColorPicker4() only accesses 3 floats if ImGuiColorEditFlags_NoAlpha flag is set.
// FIXME: we adjust the big color square height based on item width, which may cause a flickering feedback loop (if automatic height makes a vertical scrollbar appears, affecting automatic width..)
bool ImGui::ColorDrawHorizontalHSV(const char* label, float col1[3], float col2[3])
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	ImDrawList* draw_list = window->DrawList;
	...
	//bool alpha_bar = (flags & ImGuiColorEditFlags_AlphaBar) && !(flags & ImGuiColorEditFlags_NoAlpha);
	float square_sz = GetFrameHeight();
	float bars_height = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
	float sv_picker_size = ImMax(bars_height * 1, CalcItemWidth() - (1) * (bars_height + style.ItemInnerSpacing.x)); // Saturation/Value picking box
	ImVec2 picker_pos = window->DC.CursorPos;
	float bar0_pos_x = picker_pos.x;
	float bar1_pos_x = bar0_pos_x /*+ bars_height*/ + style.ItemInnerSpacing.x;
	float bars_triangles_half_sz = (float)(int)(bars_height * 0.20f);
	...
	ColorConvertRGBtoHSV(col1[0], col1[1], col1[2], H, S, V);
	ColorConvertRGBtoHSV(col2[0], col2[1], col2[2], H2, S2, V2);
	...
	{
		float newValue;
		newValue = ImSaturate((io.MousePos.x - picker_pos.x) / (sv_picker_size - 1));
		if (H != newValue || H2 != newValue)
		{
			if (fabsf(H - newValue) < fabsf(H2 - newValue))
			{
				H = newValue;
	...
	if (value_changed_h)
		ColorConvertHSVtoRGB(H >= 1.0f ? H - 10 * 1e-6f : H, S > 0.0f ? S : 10 * 1e-6f, V > 0.0f ? V : 1e-6f, col1[0], col1[1], col1[2]);

	if (value_changed)
		ColorConvertHSVtoRGB(H2 >= 1.0f ? H2 - 10 * 1e-6f : H2, S2 > 0.0f ? S2 : 10 * 1e-6f, V2 > 0.0f ? V2 : 1e-6f, col2[0], col2[1], col2[2]);

	// Render Hue Bar
	//float bar_pos_x = 0;
	for (int i = 0; i < 6; ++i)
		draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x + i * (sv_picker_size / 6), picker_pos.y),
			ImVec2(bar0_pos_x + (i + 1) * (sv_picker_size / 6), picker_pos.y + bars_height),
			hue_colors[i],// up-left
			hue_colors[i + 1], //up-right
			hue_colors[i + 1], //bot-right
			hue_colors[i]);//bot-left  


	//Min triangle
	 float bar0_line_x = (float)(int)(picker_pos.x + H * sv_picker_size + 0.5f);
	 float bar0_line_y = (float)(int)(picker_pos.y);
	//RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + sv_picker_size, picker_pos.y + bars_height), 0.0f);
	RenderArrowsForHorizontalBar(draw_list, 
								ImVec2(bar0_line_x, bar0_line_y - 1),
								ImVec2(bars_triangles_half_sz, bars_triangles_half_sz + 1), 
								bars_height + 2.0f);
		
	//Max triangle
	float bar0_line_x1 = (float)(int)(picker_pos.x + H2 * sv_picker_size + 0.5f);
	float bar0_line_y1 = (float)(int)(picker_pos.y);
	//RenderFrameBorder(ImVec2(bar0_line_x1, picker_pos.y), ImVec2(bar0_line_x1 + sv_picker_size , picker_pos.y + bars_height), 0.0f);
	RenderArrowsForHorizontalBar(draw_list, 
								ImVec2(bar0_line_x1, bar0_line_y1 - 1),
								ImVec2(bars_triangles_half_sz, bars_triangles_half_sz + 1), 
								bars_height + 2.0f);
	...
	return (value_changed || value_changed_h);
}
